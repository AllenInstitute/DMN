...
st_dict = dat.groupby('match_A')['source'].unique() #may need to change match_A match_B
...
matches = dat[dat['match_A'] == isid] #may need to switch match_A and match_B - could even have both
df = pd.DataFrame({'experiment': np.concatenate((matches['match_B'].values,
                                                     [isid]))})
...
pivot_dfm = meandat[meandat['method'] == 'target-defined'].pivot(
        ...
        columns = 'layer', 
        values = 'value')
pivot_dfm['label'] = [st_dict[experiment][0] for experiment in pivot_dfm.index]
