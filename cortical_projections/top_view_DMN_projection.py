...
def main():
    ...
    template_file = '/allen/programs/celltypes/workgroups/mousecelltypes/T503_Connectivity_in_Alzheimer_Mice/Jennifer/cluster_code/cortical_projections/ica_all_05_icasso_iter_1000_comp_1_mask_z_1_allen_masked_sym_thresh_2.nrrd'
    ...
    template = scipy.ndimage.zoom(template_100, 10, order=4)
    template[(0,0,0)] = 0
    ...
    nrrd.write( os.path.join(args.output_file, 'DMN_mask_cortical_projection_10.nrrd'), output )
    nrrd.write( os.path.join(args.output_file, 'DMN_mask_cortical_projection_25.nrrd'), ds_output )
    im = Image.fromarray(np.uint8(plt.cm.gray(ds_output))*255)
